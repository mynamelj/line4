...
namespace MES.Manager
{
    public partial class DataManager
    {
        ...
        public async void ScanCheckIn(string stationNumber)
        {
            try
            {
                int iNumber = Convert.ToInt32(stationNumber) - 1;
                ...
                while (!SetHelper.IsRestart[iNumber])
                {
                    ...
                    if (Application.Current != null)
                    {
                        await Application.Current.Dispatcher.BeginInvoke(() =>
                        {
                            if (!SetHelper.IsOpen[iNumber])
                            {
                                ...
                                PopupSeqView window;
                                if (stationName.Contains("OP3045") && stationNumber == "3")
                                {
                                    window = new PopupSeqView(stationName, iNumber, stationName + "\r\n\r\n请扫描后端盖码进站");
                                }
                                else
                                {
                                    window = new PopupSeqView(stationName, iNumber, "");
                                }

                                //PopupSeqView window = new PopupSeqView(stationName, iNumber, "");
                                //window.Height = 100;
                    ...
                    {
                        ProductCheckIn(stationNumber, SetHelper.NowMaterialCode[iNumber]);
            ...
            try
            {
                ...
                if (dic.Count != 0)
                {
                    ...
                    if (data is object[] datarray)
                    {
                        ...
                        //分类讨论
                        //标签有以1_ 2_开头的，代表有多个螺栓或者有垫片
                        //bool isManyBolts = false;
                        //foreach (var key in TagNameList)
                        //{
                        //    if (key.StartsWith("1_") || key.StartsWith("2_") || key.StartsWith("3_") || key.StartsWith("4_"))
                        //    {
                        //        isManyBolts = true;
                        //        break;
                        //    }
                        ...
                        //if (isManyBolts)
                        //{
                        //    //string jsonPLC = File.ReadAllText(SetHelper.plcpath);
                        //    //PLCSettingModel PLCSetting = JsonConvert.DeserializeObject<PLCSettingModel>(jsonPLC);
                        //    PLCSettingModel PLCSetting = SetHelper.LoadConfig<PLCSettingModel>(SetHelper.plc, SetHelper.NowProduct);
                        //    var getGroupModel = PLCSetting.ListGroup.FirstOrDefault(m => m.GroupName.Contains("OEE数据收集"));
                        //    //多螺栓工位再分工位讨论
                        //    //OP2080 1螺栓+1垫片
                        //    //OP5090 3螺栓
                        //    //OP3045 工站一2种螺栓，工站三1种螺栓
                        //    if (stationName.ToUpper().Contains("OP2080"))
                        //    {
                        //        for (int i = 0; i < datarray.Length; i++)
                        //        {
                        //            SetHelper.ListPLCMessage.ShowInfoQueue($"{stationName} 读到OEE参数:{TagNameList[i]}为{datarray[i]}");
                        //            PLCTag tag = SetHelper.PLCSetting.ListGroup.FirstOrDefault(x => x.GroupType == PLCGroupName.OeeGroup.ToString())?.ListTag.FirstOrDefault(x => TagNameList[i].Contains(x.TagName));
                        ...
                        //            if (dC_Info.Item.Contains("Toss_Count") || dC_Info.Item.Contains("TossRate"))
                        //            {
                        //                var getTossCode = getGroupModel.ListTag.FirstOrDefault(x => x.TagName == dC_Info.Item);
                        //                if (getTossCode.TagDesc.Contains("垫片"))
                        //                {
                        //                    int lightID = 0;
                        //                    //垫片
                        //                    if (SetHelper.siemens.ReadItem(PLCGroupName.ReadGroup, "灯号_" + number, ref obj))
                        //                    {
                        //                        lightID = obj.Obj2Int();
                        //                    }
                        //                    if (lightID == 0) SetHelper.ListPLCMessage.ShowInfoQueue($"{stationName} 读OEE参数绑定的灯号失败");
                        //                    var materaIlWithGasket = materails.FirstOrDefault(m => (m.MaterialName.Contains("垫片") && m.LightNumber == lightID));
                        //                    if (materaIlWithGasket != null)
                        //                    {
                        //                        if (dC_Info.Item.StartsWith("1_") || dC_Info.Item.StartsWith("2_") || dC_Info.Item.StartsWith("3_"))
                        //                            dC_Info.Item = $"{dC_Info.Item.Substring(dC_Info.Item.IndexOf('_') + 1)}_{materaIlWithGasket.GlueCode.Split('|')[0].ToString()}";
                        //                        else
                        //                            dC_Info.Item = $"{dC_Info.Item}_{materaIlWithGasket.GlueCode.Split('|')[0].ToString()}";
                        //                    }
                        //                }
                        //                else
                        //                {
                        //                    //螺栓
                        //                    var materaIlWithBolts = materails.FirstOrDefault(m => m.MaterialName.Contains("螺栓"));
                        //                    if (materaIlWithBolts != null)
                        //                    {
                        //                        if (dC_Info.Item.StartsWith("1_") || dC_Info.Item.StartsWith("2_") || dC_Info.Item.StartsWith("3_"))
                        //                            dC_Info.Item = $"{dC_Info.Item.Substring(dC_Info.Item.IndexOf('_') + 1)}_{materaIlWithBolts.GlueCode.Split('|')[0].ToString()}";
                        //                        else
                        //                            dC_Info.Item = $"{dC_Info.Item}_{materaIlWithBolts.GlueCode.Split('|')[0].ToString()}";
                        //                    }
                        //                }
                        ...
                        //                //螺栓
                        //                //匹配前三个字
                        //                var materaIlWithBolts = materails.FirstOrDefault(m => (m.MaterialName.Contains(getTossCode.TagDesc.Substring(0, 3))));
                        //                if (materaIlWithBolts != null)
                        //                {
                        //                    if (dC_Info.Item.StartsWith("1_") || dC_Info.Item.StartsWith("2_") || dC_Info.Item.StartsWith("3_") || dC_Info.Item.StartsWith("4_"))
                        //                        dC_Info.Item = $"{dC_Info.Item.Substring(dC_Info.Item.IndexOf('_') + 1)}_{materaIlWithBolts.GlueCode.Split('|')[0].ToString()}";
                        //                    else
                        //                        dC_Info.Item = $"{dC_Info.Item}_{materaIlWithBolts.GlueCode.Split('|')[0].ToString()}";
                        //                }
                        //                else SetHelper.ListPLCMessage.ShowInfoQueue($"{stationName} OEE流程中查找不到多螺栓对应的料号");
                        //            }
                        //            oeeInfoList.Add(dC_Info);
                        //        }
                        //    }
                        ...
                        //        if (dC_Info.Item.Contains("Toss_Count") || dC_Info.Item.Contains("TossRate"))
                        //        {
                        //            var materaIlWithBolts = materails.FirstOrDefault(m => m.MaterialName.Contains("螺栓"));//需要确认所有含螺栓工位的单物料命名
                        //            dC_Info.Item = $"{dC_Info.Item}_{materaIlWithBolts.GlueCode}";
